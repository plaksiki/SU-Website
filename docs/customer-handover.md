...
- **Current state in one paragraph:** The current release is **MVP v2.0.0** (Assignment 5) - a React frontend paired with a Java Spring Boot backend, deployed via Docker Compose on the production VM `10.93.26.192`. The earlier MVP v0 (static HTML/CSS) was hosted on Netlify and is now a deprecated legacy version.
